...
/**
 * Based on EncryptedSharedPreferencesTest.java from AndroidX Crypto - v1.1.0-alpha07
 *
 * <p>Permalink: <a
 * href="https://android.googlesource.com/platform/frameworks/support/+/e50caacef9794c6c1d05ed647347a01b06b96930/security/security-crypto/src/androidTest/java/androidx/security/crypto/EncryptedSharedPreferencesTest.java">e50caac</a>
 */
@MediumTest
...
public class EncryptedSharedPreferencesTest {
    ...
    public void setup() throws Exception {
        ...
        SharedPreferences notEncryptedSharedPrefs =
                mContext.getSharedPreferences(PREFS_FILE, MODE_PRIVATE);
        ...
        SharedPreferences encryptedSharedPrefs =
                mContext.getSharedPreferences("TinkTestPrefs", MODE_PRIVATE);
        encryptedSharedPrefs.edit().clear().commit();

        filePath = mContext.getFilesDir().getParent() + "/shared_prefs/" + "TinkTestPrefs";
        deletePrefFile = new File(filePath);
        deletePrefFile.delete();
    ...
    public void testWriteSharedPrefs() throws Exception {
        ...
        Assert.assertEquals(
                stringSetTestKey + " has the wrong value",
                ((ArraySet<String>) stringSetValue).valueAt(0),
                stringSetTestValue);

        // Int Test Assertion
        Assert.assertEquals(
                intTestKey + " has the wrong value",
                intTestValue,
                sharedPreferences.getInt(intTestKey, 0));

        // Long Test Assertion
        Assert.assertEquals(
                longTestKey + " has the wrong value",
                longTestValue,
                sharedPreferences.getLong(longTestKey, 0L));

        // Boolean Test Assertion
        Assert.assertEquals(
                booleanTestKey + " has the wrong value",
                booleanTestValue,
                sharedPreferences.getBoolean(booleanTestKey, false));

        // Float Test Assertion
        Assert.assertEquals(floatTestValue, sharedPreferences.getFloat(floatTestKey, 0.0f), 0.0f);

        // Null Key Test Assertion
        Assert.assertEquals(
                nullKey + " has the wrong value",
                nullStringValue,
        ...
        Assert.assertNull(
                nullKey + " should have been removed.", sharedPreferences.getString(nullKey, null));

        Assert.assertFalse(nullKey + " should not exist", sharedPreferences.contains(nullKey));
        ...
        editor.commit();
        Assert.assertNull(
                null + " should not have a value", sharedPreferences.getString(null, null));

        // Null StringSet Key and value Test Assertion
        Assert.assertNull(
                null + " should not have a value", sharedPreferences.getStringSet(null, null));
        ...
        Assert.assertTrue("Get all should have supplied " + twiceKey, all.containsKey(twiceKey));

        Assert.assertFalse(
                "Get all should have removed " + KEY_KEYSET_ALIAS,
                all.containsKey(KEY_KEYSET_ALIAS));

        Assert.assertFalse(
                "Get all should have removed " + VALUE_KEYSET_ALIAS,
        ...
        Assert.assertTrue(
                "Access to " + KEY_KEYSET_ALIAS + " should have been blocked.", exceptionThrown);
        ...
        Assert.assertTrue(
                "Access to " + VALUE_KEYSET_ALIAS + " should have been blocked.", exceptionThrown);
        exceptionThrown = false;
        ...
        Assert.assertEquals(
                "Data should be equal", "New", sharedPreferences.getString("New Data", null));
        ...
        Assert.assertEquals("Get all size should be equal", 0, sharedPreferences.getAll().size());
    }
    ...
    public void testWriteSharedPrefsTink() throws Exception {
        ...
        KeysetHandle daeadKeysetHandle =
                new AndroidKeysetManager.Builder()
                        ...
                        .withSharedPref(
                                ...
                                tinkTestPrefs)
                        .withMasterKeyUri(
                                MasterKey.KEYSTORE_PATH_URI + "_androidx_security_master_key_")
                        .build()
                        .getKeysetHandle();
        ...
        Assert.assertEquals("String should have been equal to original", actualValue, testValue);
    }
    ...
    public void testOnSharedPreferenceChangeListenerWithAdd() throws Exception {
        ...
        final SharedPreferences.OnSharedPreferenceChangeListener addListener =
                new SharedPreferences.OnSharedPreferenceChangeListener() {
                    ...
                    public void onSharedPreferenceChanged(
                            SharedPreferences sharedPreferences, String key) {
                        Assert.assertEquals(
                                key + " should match " + stringTestKey, stringTestKey, key);
                        Assert.assertEquals(
                                stringTestValue, sharedPreferences.getString(stringTestKey, null));
        ...
        Assert.assertTrue(
                stringAdd + " callback should have been invoked.",
    ...
    public void testOnSharedPreferenceChangeListenerWithEdit() throws Exception {
        ...
        Assert.assertTrue(
                stringEdit + " callback should have been invoked.",
    ...
    public void testOnSharedPreferenceChangeListenerWithRemove() throws Exception {
        ...
        Assert.assertTrue(
                stringRemove + " callback should have been invoked.",
